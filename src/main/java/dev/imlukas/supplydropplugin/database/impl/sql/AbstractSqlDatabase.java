package dev.imlukas.supplydropplugin.database.impl.sql;

import dev.imlukas.supplydropplugin.database.AbstractDatabase;
import dev.imlukas.supplydropplugin.database.impl.sql.functions.ResultSetFunction;
import dev.imlukas.supplydropplugin.database.impl.sql.statement.StatementObject;
...
public abstract class AbstractSqlDatabase extends AbstractDatabase {
    ...
    protected <T> CompletableFuture<T> runQueryAsync(String query , ResultSetFunction<T> function) {
        return associate(() -> runQuery(query, List.of(), function));
    }
